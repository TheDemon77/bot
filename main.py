...
def extract_chapter_number(text):
    ...
    # 1. الأولوية الأولى: البحث عن علامات الفصول الصريحة (الأكثر دقة)
    # يكتشف: ch102, ch 102, chapter 102, #102, c102
    markers = [
    ...
    # 2. الأولوية الثانية: البحث عن نطاق مدمج (مثل 0-20)
    # يتم البحث عنه فقط إذا لم نجد فصل صريح
    range_match = re.search(r'(\d+\s*-\s*\d+)', text)
    if range_match: 
        return range_match.group(1).replace(" ", "")

    # 3. الأولوية الثالثة: البحث عن كلمة Volume/Vol لأنها غالباً في المدمج
    vol_match = re.search(r'vol(?:ume|\.)?\s*(\d+(\.\d+)?)', text_clean)
    if vol_match:
        return vol_match.group(1)
    
    # 4. الملاذ الأخير: البحث عن أي رقم عائم في النص (للملفات التي هي مجرد "102.pdf")
